...
extension UIImage {
    func quantizeImage(colors: [Color]? = nil, numberOfColors: Int? = 2, quality: UIImageQuality? = .lowest) -> QuantizedImage? {
        var quantizedColors: [Color]
        if colors != nil {
            quantizedColors = colors!
        }
        else {
            quantizedColors = self.getDominantColors(numberOfColors: numberOfColors!, quality: quality!)
        }
